...
REM
schtasks /RU "%SERVICE_ACCOUNT%" /CREATE /SC ONEVENT /EC Application /MO *[System/EventID=777] /TN "Syncthing" /TR "%~dps0syncthing.exe serve --no-console --no-browser --no-upgrade --home=\"%ProgramData%\Syncthing\" --logfile=\"%ProgramData%\Syncthing\syncthing.log\" %*" /F
schtasks /RUN /TN "Syncthing"
REM
REM Open log tail in separate window.
start "Syncthing Log" powershell -ExecutionPolicy "ByPass" "Get-Content -Path \"%ProgramData%\Syncthing\syncthing.log\" -Wait"
REM
