...
SET "SYNCTHING_APPDATA=%ProgramData%\Syncthing"
SET CONFIG_XML="%SYNCTHING_APPDATA%\config.xml"
SET "TILDE_ALIAS_FOLDER=%SystemDrive%\Syncthing"
REM
SET "STHOMEDIR=%SYNCTHING_APPDATA%"
REM 
REM Always stop service before a test.
IF NOT DEFINED SYNCTHING_APPDATA call :logAdd "[ERROR] SYNCTHING_APPDATA not defined. Stop." & goto :eof
IF "%SYNCTHING_APPDATA%" == "\" call :logAdd "[ERROR] SYNCTHING_APPDATA invalid. Stop." & goto :eof
IF "%SYNCTHING_APPDATA%" == "." call :logAdd "[ERROR] SYNCTHING_APPDATA invalid. Stop." & goto :eof
IF "%SYNCTHING_APPDATA%" == "%SystemDrive%" call :logAdd "[ERROR] SYNCTHING_APPDATA invalid. Stop." & goto :eof
IF "%DEBUG_MODE%" == "1" net stop "Syncthing" & RD /S /Q "%SYNCTHING_APPDATA%"
IF NOT EXIST "%SYNCTHING_APPDATA%" MD "%SYNCTHING_APPDATA%"
REM 
REM Note:
...
copy /y %CONFIG_XML% "%SYNCTHING_APPDATA%\%CONFIG_XML_BACKUP%"
...
REM
REM 	defaults: folder
call :editConfigXml "(.*<folder id=\`\` label=\`.*\` path=)\`\`" "$1`%TILDE_ALIAS_FOLDER%`"
REM
...
REM 
"%SYNCTHING_EXE%" generate
call :storeLocalDeviceId
IF "%DEBUG_MODE%" == "1" copy /y "%SYNCTHING_APPDATA%\config.xml" "%SYNCTHING_APPDATA%\generateKeysAndConfig_result.xml" 
REM 
goto :eof
...
REM 	[IN] SYNCTHING_PATH
REM 
"%SYNCTHING_EXE%" device-id > "%SYNCTHING_APPDATA%\device_id.txt"
REM 
goto :eof
...
REM 
SET LOCAL_DEVICE_ID=
IF NOT EXIST "%SYNCTHING_APPDATA%\device_id.txt" call :storeLocalDeviceId
FOR /F "tokens=1" %%A IN ('TYPE "%SYNCTHING_APPDATA%\device_id.txt" 2^>NUL:') DO SET "LOCAL_DEVICE_ID=%%A"
...
call :execTakeown "%SYNCTHING_APPDATA%"
icacls "%SYNCTHING_APPDATA%" /inheritance:r 1> NUL:
icacls "%SYNCTHING_APPDATA%" /grant "%ADMIN_GROUP_SID%":(OI)(CI)F /T 1> NUL:
icacls "%SYNCTHING_APPDATA%" /grant "%USER_LOCAL_SERVICE_SID%":(OI)(CI)F /T 1> NUL:
icacls "%SYNCTHING_APPDATA%" /grant "%USER_NETWORK_SERVICE_SID%":(OI)(CI)F /T 1> NUL:
icacls "%SYNCTHING_APPDATA%" /grant "SYSTEM":(OI)(CI)F /T 1> NUL:
REM 
goto :eof
...
SET SYNCTHING_EXE=
FOR /f "tokens=*" %%i in ('powershell "(Get-Item -LiteralPath '%SYNCTHING_EXE_SHORT%').FullName"') DO SET SYNCTHING_EXE=%%i
